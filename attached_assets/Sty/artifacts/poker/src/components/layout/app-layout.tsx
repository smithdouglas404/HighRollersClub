...
import { Hexagon, LayoutDashboard, Search, Trophy, Users, LogOut } from "lucide-react";
            ...
            <div className="flex items-center gap-3">
              <Link href="/profile" className="flex items-center gap-3 glass-panel !p-2 !rounded-full pr-4 hover:border-primary/50 transition-all cursor-pointer">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-[10px] text-primary font-display">{formatChips(balance)}</span>
                </div>
              </Link>
              <button
                onClick={async () => {
                  try { await fetch(`${import.meta.env.BASE_URL.replace(/\/$/, "")}api/auth/logout`, { method: "POST", credentials: "include" }); } catch {}
                  await signOut(auth);
                }}
                className="p-2 rounded-lg text-muted-foreground hover:text-destructive hover:bg-white/5 transition-all"
                title="Sign out"
              >
