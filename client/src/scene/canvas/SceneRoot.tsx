import { Environment } from "@react-three/drei";
import { CameraRig } from "./CameraRig";
import { TableBase } from "../table/TableBase";
import { FeltSurface } from "../table/FeltSurface";
import { OuterMetalRing } from "../table/OuterMetalRing";
import { InnerGoldRing } from "../table/InnerGoldRing";
import { SeatRingGroup } from "../table/SeatRing";
import { PostFx } from "../fx/PostFx";

interface SceneRootProps {
  quality?: "low" | "medium" | "high" | "cinematic";
  activeSeat?: number;
  winnerSeat?: number;
}

export function SceneRoot({ quality = "high", activeSeat, winnerSeat }: SceneRootProps) {
  return (
    <>
      <CameraRig />

      <Environment
        preset="night"
        environmentIntensity={0.4}
        environmentRotation={[0, Math.PI / 4, 0]}
      />

      <spotLight
        position={[0, 7, 0]}
        angle={0.45}
        penumbra={0.9}
        intensity={3.0}
        color="#fff5e0"
        castShadow
        shadow-mapSize-width={quality === "low" ? 512 : 2048}
        shadow-mapSize-height={quality === "low" ? 512 : 2048}
        shadow-bias={-0.0003}
      />

      <ambientLight intensity={0.08} color="#8ecbff" />

      <hemisphereLight args={["#1a1a30", "#050508", 0.2]} />

      <pointLight position={[-4.5, 2.5, 0]} intensity={0.4} color="#58f1ff" distance={12} decay={2} />
      <pointLight position={[4.5, 2.5, 0]} intensity={0.4} color="#58f1ff" distance={12} decay={2} />

      <pointLight position={[0, 3, -3]} intensity={0.2} color="#f2c660" distance={10} decay={2} />

      <pointLight position={[0, -1, 0]} intensity={0.1} color="#1a1205" distance={8} />

      <group>
        <TableBase />
        <FeltSurface />
        <OuterMetalRing />
        <InnerGoldRing />
        <SeatRingGroup
          count={10}
          activeSeat={activeSeat}
          winnerSeat={winnerSeat}
        />
      </group>

      <group position={[0, 0.02, 0]}>
        {[-0.48, -0.24, 0, 0.24, 0.48].map((xOff, i) => (
          <mesh key={`card-slot-${i}`} position={[xOff, 0, 0]}>
            <boxGeometry args={[0.18, 0.003, 0.26]} />
            <meshStandardMaterial
              color="#0d2818"
              transparent
              opacity={0.3}
              roughness={0.9}
            />
          </mesh>
        ))}
      </group>

      <PostFx quality={quality} />
    </>
  );
}
