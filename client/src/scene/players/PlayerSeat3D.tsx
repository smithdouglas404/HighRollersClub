...
export function PlayerSeat3D({ seatIndex, position }: PlayerSeat3DProps) {
  ...
  const { frameGeo, frameMat, backGeo, backMat } = useMemo(() => {
    // Frame border as a torus-like shape (simplified: ring)
    const fGeo = new THREE.RingGeometry(0.12, 0.135, 4);
    // Rotate ring to look like rounded square
    const fMat = new THREE.MeshStandardMaterial({
      color: new THREE.Color(isWinner ? "#3d2e0a" : "#1a3040"),
      emissive: new THREE.Color(isWinner ? "#f2c660" : "#58f1ff"),
      emissiveIntensity: isActive ? 1.5 : isWinner ? 1.2 : 0.4,
      roughness: 0.3,
      metalness: 0.5,
      transparent: true,
      opacity: isFolded ? 0.3 : 0.9,
    });

    // Back panel (avatar background)
    const bGeo = new THREE.PlaneGeometry(0.22, 0.22);
    const bMat = new THREE.MeshStandardMaterial({
      color: new THREE.Color("#161d29"),
      roughness: 0.8,
      metalness: 0.1,
      transparent: true,
      opacity: isFolded ? 0.3 : 0.85,
    });
